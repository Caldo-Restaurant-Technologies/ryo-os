...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let node_handler = tokio::spawn(async move {
        let node = Node::new(ClearCoreMotor::new(NODE_D_MOTOR_ID, CC_STEP_COUNTS, drive_2_tx.clone()));
        node.actor(716620, node_rx).await.unwrap();
    });
    
    let gantry_handler = tokio::spawn(async move{
        gantry(ClearCoreMotor::new(GANTRY_MOTOR_ID, 800, drive_1_tx.clone()), gantry_rx).await
    });
