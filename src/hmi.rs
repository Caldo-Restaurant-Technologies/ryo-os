use crate::config::{
    get_inputs, get_motors, BAG_BLOWER, BAG_ROLLER_MOTOR_ID, BAG_ROLLER_PE, CC_STEP_COUNTS,
    GANTRY_MOTOR_ID, GRIPPER_ACTUATOR, GRIPPER_MOTOR_ID, RYO_INPUT_COUNT, RYO_MOTOR_COUNT,
...
use control_components::subsystems::bag_handling::{load_bag, BagDispenser, BagGripper};
...
pub async fn ui_request_handler(
    ...
    let hatch_a_actuator = RelayHBridge::new(tx_s.drive_senders.0.clone(), (2, 3), 3);
    let hatch_b_actuator = RelayHBridge::new(tx_s.drive_senders.0.clone(), (4, 5), 4);
    let hatch_c_actuator = RelayHBridge::new(tx_s.drive_senders.1.clone(), (0, 1), 3);
    let hatch_d_actuator = RelayHBridge::new(tx_s.drive_senders.1.clone(), (2, 3), 4);
    ...
    match (req.method(), req.uri().path()) {
        ...
        (&Method::POST, "/gripper") => {
            let bag_gripper = BagGripper::new(
                ...
                SimpleLinearActuator::new(tx_s.drive_senders.1.clone(), GRIPPER_ACTUATOR, 0),
                [-0.4, 0.8, -0.4].to_vec(),
            );
        ...
        (&Method::POST, "/load_bag") => {
            ...
            let task = tokio::spawn(async move {
                ...
                gantry
                    .wait_for_move(Duration::from_secs_f64(0.5))
                    .await
                    .unwrap();
        ...
        (&Method::POST, "/hatch") => {
            ...
            if body.len() >= 2 {
                ...
                let hatch = match hatch_id {
                    0 => Some(Hatch::new(hatch_a_actuator, Duration::from_secs_f64(3.))),
                    1 => Some(Hatch::new(hatch_b_actuator, Duration::from_secs_f64(3.))),
                    2 => Some(Hatch::new(hatch_c_actuator, Duration::from_secs_f64(1.7))),
                    3 => Some(Hatch::new(hatch_d_actuator, Duration::from_secs_f64(3.))),
                    _ => None,
                };
                if hatch.is_some() {
                    if operation == b'o' {
...
pub async fn ui_server(txs: UISenders) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
